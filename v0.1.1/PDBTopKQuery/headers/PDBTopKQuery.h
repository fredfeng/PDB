...
public:
	...
	//
	PDBTopKQuery (string queryString, const PDBFeatureList &query, PDBCatalogPtr catalog);
	...
	void aggregate (PDBData &queryMe);
	string display ();
	bool wasAnError (string &message);
...
private:
	...
	// the query string in a c-like rep
	char *input;
