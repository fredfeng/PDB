...
public:
    ...
    // copy from another PDBData object
    PDBData (const PDBData &fromMe);

    // assignment of PDBData objects
    PDBData &operator = (const PDBData &other);
    ...
    // returnVal).  The deserializaion is performed using the two machines provided
