...
public:
	...
	//
	PDBTopKQuery (const string &queryString, const PDBFeatureList &query, PDBCatalog &catalog);

	// constructor
	PDBTopKQuery ();

	// constructor that accepts the catalog
	PDBTopKQuery (PDBCatalog &catalog);
...
private:
	...
	string compile (bool &foundError);
	...
	// the query string in a c-like rep with *two* trailing NULLs
	string input;
