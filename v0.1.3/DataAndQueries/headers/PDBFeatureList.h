...
public:
	...
	// the features; success tells us whether we were successful
	void *deSerialize (void *fromHere, size_t &size, PDBFeatureTypeDeserializationMachine &machine, bool &success);

	// like the above; this one will not fail, however; in the case we cannot match the
	// type, a FlatFeatureType is created that simply stored size bytes read from fromHere
