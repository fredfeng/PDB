...
FilesToBuildFrom *ReadExecsFromFile (FILE *fromMe, int &curPosInOutput) {
        ...
        char buffer[1000];
        auto output = new FilesToBuildFrom[256];
...
void SortFile (const char *inputFile, const char *outputFile) {
        ...
        auto allLines = new char *[numLines];
...
void BuildMakefile (const char *makefileName, const char *prelude, const char *headerIncludeDirectories,
        const char *m4IncludeDirectories, const char *m4FilesForHeaders, const char *m4FilesForCPlusPlus,
        ...
        if (headerIncludeDirectories != nullptr) {
        ...
        // add all of the m4 include directories to the 2nd half of the makefile
        if (m4IncludeDirectories != nullptr) {
                fprintf (suffix, "# here we add additional m4 include directories\n");
                FILE *temp = fopen(m4IncludeDirectories, "r");
                char buffer[512];
                while (GetLine (buffer, temp)) {
                        fprintf (suffix, "M4INCLUDE += -I %s\n", buffer);
                }
                fprintf (suffix, "\n\n");
                fclose (temp);
        }
        ...
        // add rules to make all of the .h files from .h.m4 files
        if (m4FilesForHeaders != nullptr) {
                fprintf (suffix, "# these are rules for compiling .h.m4 files\n");
                FILE *temp = fopen(m4FilesForHeaders, "r");
                char buffer[512];
                while (GetLine (buffer, temp)) {
                        char *target = strdup (buffer);
                        target[strlen(target) - 3] = 0;
                        char *temp = replace (target, "/m4/private/", "/headers/");
                        free (target);
                        target = replace (temp, "/m4/", "/headers/");
                        free (temp);
                        fprintf (suffix, "%s: %s\n\t$(M4) $(M4INCLUDE) %s > %s\n\n", target, buffer, buffer, target);
                        free (target);
                }
                fclose (temp);
                temp = fopen(m4FilesForHeaders, "r");
                fprintf (suffix, "# this will compile all of the .h.m4 files\nallHFromM4:");
                while (GetLine (buffer, temp)) {
                        char *target = strdup (buffer);
                        target[strlen(target) - 3] = 0;
                        char *temp = replace (target, "/m4/private/", "/headers/");
        ...
        // add rules to make all of the .h files from .h.m4 files
        if (m4FilesForCPlusPlus != nullptr) {
                fprintf (suffix, "# these are rules for compiling .h.cc files\n");
                FILE *temp = fopen(m4FilesForCPlusPlus, "r");
                char buffer[512];
                while (GetLine (buffer, temp)) {
                        char *target = strdup (buffer);
                        target[strlen(target) - 3] = 0;
                        char *temp = replace (target, "/m4/private/", "/source/");
                        free (target);
                        target = replace (temp, "/m4/", "/source/");
                        free (temp);
                        fprintf (suffix, "%s: %s\n\t$(M4) $(M4INCLUDE) %s > %s\n\n", target, buffer, buffer, target);
                        free (target);
                }
                fclose (temp);
                temp = fopen(m4FilesForCPlusPlus, "r");
                fprintf (suffix, "# this will compile all of the .cc.m4 files\nallCPlusPlusFromM4:");
                while (GetLine (buffer, temp)) {
                        char *target = strdup (buffer);
                        target[strlen(target) - 3] = 0;
                        char *temp = replace (target, "/m4/private/", "/source/");
        ...
        if (filesToBuildDependenciesFor != nullptr && dependencyFile != nullptr) {
                ...
                while (GetLine (buffer, temp)) {
                        ...
                        fprintf (suffix, "\t$(CXX) $(CXXFLAGS) $(INCLUDE) -M %s >> %s.dep\n", buffer, buffer);
                        fprintf (suffix, "\techo -e \"\t\\$$(CXX) \\$$(CXXFLAGS) \\$$(INCLUDE) -o %s/object/%s -c %s\\n\" >> %s.dep\n\n",
        ...
        for (int i = 0; i < numProjects; i++) {
                ...
                while (GetLine (buffer, temp)) {
                        ...
                        fprintf (suffix, "\n\tln -s ../%s Headers%s/", buffer, myProjects[i].target + j);
                }
                fclose (temp);


                // now give the rule to do the linking
                fprintf (suffix, "\n\t$(CXX) -o %s", myProjects[i].target);
        ...
        if (m4FilesForHeaders != nullptr || m4FilesForCPlusPlus != nullptr || filesToBuildDependenciesFor != nullptr) {
        ...
        if (m4FilesForHeaders != nullptr) {
                ...
                char buffer[512];
                while (GetLine (buffer, temp)) {
                        char *target = replace (buffer, "/m4/", "/headers/");
                        char *target2 = replace (target, "/m4/private/", "/headers/");
                        target2[strlen(target2) - 3] = 0;
        ...
        if (m4FilesForCPlusPlus != nullptr) {
                ...
                char buffer[512];
                while (GetLine (buffer, temp)) {
                        char *target = replace (buffer, "/m4/", "/source/");
                        char *target2 = replace (target, "/m4/private/", "/source/");
                        target2[strlen(target2) - 3] = 0;
                        fprintf (suffix, "%s ", target2);
                        free (target);
                        free (target2);
        ...
        if (filesToBuildDependenciesFor != nullptr) {
                FILE *temp = fopen(filesToBuildDependenciesFor, "r");
                char buffer[512];
                while (GetLine (buffer, temp)) {
        ...
        if (rulesForObjectFiles != nullptr) {
...
int main (int numArgs, char **args) {
        ...
        int code = 0;
        #define PROB_M4_TO_H 1
        #define PROB_M4_TO_CC 2
        ...
        // "CPlusPlusM4Files" will include all m4 files that will be compiled to a .cc file
        system ("rm CPlusPlusM4Files 2>/dev/null");
        system ("ls */m4/*.cc.m4 */m4/private/*.cc.m4 > CPlusPlusM4Files 2>/dev/null");
        system ("touch CPlusPlusM4Files");

        // "HeaderM4Files" will include all m4 files that will be compiled to a .h file
        system ("rm HeaderM4Files 2>/dev/null");
        system ("ls */m4/*.h.m4 */m4/private/*.h.m4 > HeaderM4Files 2>/dev/null");
        system ("touch HeaderM4Files");
        ...
        // "AllUsefulCPlusPlusM4Files" will include all m4 files that will be compiled to
        // a .cc file, AND are part of some useful directory
        Semijoin("CPlusPlusM4Files", "AllUsefulDirectories", "AllUsefulCPlusPlusM4Files");
        Semijoin("HeaderM4Files", "AllUsefulDirectories", "AllUsefulHeaderM4Files");
        ...
        // see which m4 files that will be compiled to a .cc file don't have a source directory
        // to be put into
        Strip ("AllSourceDirectories", 1, "AllSourceDirectoriesTemp");
        Strip ("AllHeaderDirectories", 1, "AllHeaderDirectoriesTemp");
        Antijoin("AllUsefulCPlusPlusM4Files", "AllSourceDirectoriesTemp", "AllThatNeedASource");
        Antijoin("AllUsefulHeaderM4Files", "AllHeaderDirectoriesTemp", "AllThatNeedAHeader");
        ...
        fclose (sourceDirs);
        if (numLines == 0) {
                cout << "Just verified that all source directories I need to compile .cc.m4 files to exist.\n";
        } else {
                cout << "Created " << numLines << " new source directories to compile .cc.m4 files to.\n";
        }
        ...
        fclose (headerDirs);
        if (numLines == 0) {
                cout << "Just verified that all header directories I need to compile .h.m4 files to exist.\n";
        } else {
                cout << "Created " << numLines << " new header directories to compile .h.m4 files to.\n";
        }

        // "M4IncludeDirectories" will include all of the directories that have pure m4 code...
        system ("rm M4IncludeDirectories 2>/dev/null");
        system ("ls -d */m4/*.m4 */m4/private/*.m4 > M4IncludeDirectories 2>/dev/null");
        system ("touch M4IncludeDirectories");
        Semijoin("M4IncludeDirectories", "AllUsefulDirectories", "Temp");
        Antijoin("Temp", "AllUsefulHeaderM4Files", "Temp2");
        Antijoin("Temp2", "AllUsefulCPlusPlusM4Files", "M4IncludeDirectories");
        Strip("M4IncludeDirectories", 0, "Temp");
        RemDups("Temp", "M4IncludeDirectories");

        // now, actually compile all of the files in "AllUsefulCPlusPlusM4Files" and "AllUsefulHeaderM4Files"
        // this is done by creating a makefile for all of them and then running it...
        BuildMakefile ("Makefile", args[2], nullptr, "M4IncludeDirectories",
                "AllUsefulHeaderM4Files", "AllUsefulCPlusPlusM4Files",
                nullptr, nullptr, nullptr, 0, nullptr);

        // now, run the makefile to compile the .cc.m4 and .h.m4 files
        cout << "Now I am going to make all of the .cc.m4 and .h.m4 files, so I can check for dependencies.\n";
        if (system("make " EXTRA_MAKE_OPTION " allHFromM4") != 0) {
                code = PROB_M4_TO_H;
                goto end;
        }

        if (system("make " EXTRA_MAKE_OPTION " allCPlusPlusFromM4") != 0) {
                code = PROB_M4_TO_CC;
                goto end;
        }
        ...
        BuildMakefile ("Makefile", args[2], "AllHeaderDirectories", nullptr, nullptr, nullptr,
                "AllCPlusPlusFiles", "DependencyFile", nullptr, 0, nullptr);
        ...
        BuildMakefile ("Makefile", args[2], "AllHeaderDirectories",
                "M4IncludeDirectories", "AllUsefulHeaderM4Files", "AllUsefulCPlusPlusM4Files",
                "AllCPlusPlusFiles", nullptr, "DependencyFile", len, output);
        ...
        system ("rm AllThatNeedASource AllUsefulCPlusPlusM4Files AllUsefulDirectories AllUsefulHeaderM4Files");
        system ("rm CPlusPlusM4Files DependencyFile FinalHeaderDirsToCreate FinalSourceDirsToCreate");
        system ("rm HeaderDirectoriesToCreate HeaderM4Files M4IncludeDirectories SourceDirectoriesToCreate");
        system ("rm AllHeaders AllRelevantHeaderFiles targs AllCPlusPlusFiles suffix Temp Temp1 Temp2 ");

        if (code == PROB_M4_TO_H) {
                cout << "\nProblem compiling some .h file from m4.";
        } else if (code == PROB_M4_TO_CC) {
                cout << "\nProblem compiling some .cc file from m4.";
